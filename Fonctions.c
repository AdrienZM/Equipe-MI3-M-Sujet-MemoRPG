...
//a MODIFIER 
char* correspondance(int numero){ //on met la correspondance d'une case et ça nous renvoie ce que ça doit afficher dans le terminal
	switch (numero){
	case 1 : return "test" ;
	case 2 : return "😁" ;
	case 3 : return "😁" ;
	...
	case 15 : return "😁" ;
	case 16 : return "😁" ;
	default :{ printf( "Erreur, non affichable");exit(1);}
	}
}
...
void afficher(Case plateau[TAILLE][TAILLE], Joueur *joueurs, Joueur *perso,int nb_joueurs) {
    ...
    for (int i = 0 ; i < TAILLE ; i++) {
        for (int j = 0 ; j < TAILLE ; j++) {
        	if(perso->pos.x == i && perso->pos.y == j){
		    	switch (perso->type){
		    		case 1: printf("\033[1;31m[%s]\033[0m", correspondance(plateau[i][j].correspond)) ; break; 
				case 2: printf("\033[1;32m[%s]\033[0m", correspondance(plateau[i][j].correspond)); break; 
				case 3: printf("\033[1;33m[%s]\033[0m", correspondance(plateau[i][j].correspond)); break; 
				case 4: printf("\033[1;34m[%s]\033[0m", correspondance(plateau[i][j].correspond)); break; 
            	}}
            	
       		else if (plateau[i][j].correspond >= 1 && plateau[i][j].correspond <= 4) {
               		switch(plateau[i][j].correspond) {
                		case 1: printf("\033[1;31m[%s]\033[0m", correspondance(plateau[i][j].correspond)) ; break; 
                    		case 2: printf("\033[1;32m[%s]\033[0m", correspondance(plateau[i][j].correspond)) ; break; 
                  	 	case 3: printf("\033[1;33m[%s]\033[0m", correspondance(plateau[i][j].correspond)) ; break; 
                ...
                printf("\033[0;36m[%s]\033[0m", correspondance(plateau[i][j].correspond)); 
            }
            else if (plateau[i][j].contenu[0] == ' ') {
                printf("    ");
            }
