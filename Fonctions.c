...
void afficher(Case plateau[TAILLE][TAILLE], Joueur *joueurs, int nb_joueurs) {
    ...
    for (int i = 0; i < TAILLE; i++) {
        for (int j = 0; j < TAILLE; j++) {
            ...
            // ÉTAPE 1 : Vérifier si un joueur est sur cette case (i, j)
            int id_joueur = -1;
            ...
            // ÉTAPE 2 : Priorité à l'affichage du joueur
            if (id_joueur != -1) {
                switch(id_joueur) {
                    case 1: printf("\033[1;31m[P1]\033[0m"); break; // Rouge
                    case 2: printf("\033[1;32m[P2]\033[0m"); break; // Vert
                    case 3: printf("\033[1;33m[P3]\033[0m"); break; // Jaune
                    case 4: printf("\033[1;34m[P4]\033[0m"); break; // Bleu
                }
            } 
            // ÉTAPE 3 : Sinon, afficher le contenu de la case
            else if (plateau[i][j].revele == 1) {
                // On affiche la valeur réelle (monstres, armes, etc.)
